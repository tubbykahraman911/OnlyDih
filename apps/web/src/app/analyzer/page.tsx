  ...
  return (
    <PhaseShell>
      <main className="mx-auto grid max-w-5xl gap-5 px-4 py-8 lg:grid-cols-[380px_1fr]">
        <Panel>
          <p className="kicker">Consent-first analysis</p>
          <h1 className="mt-2 text-2xl font-semibold">Private analyzer upload</h1>
          {!verified && (
            <div className="mt-4 rounded-md border border-[#d6a72f]/35 bg-[#fff8e6] p-3 text-sm text-[#5f3f16]">
              Verification is required before uploading. <Link className="underline" href="/verification">Check verification</Link>
            </div>
          )}
          <label className="mt-5 block text-sm">
            Private image
            <input disabled={!verified} type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" onChange={(event) => setFile(event.target.files?.[0] ?? null)} className="mt-2 block w-full text-sm text-[#5f3f16] file:mr-3 file:rounded-md file:border-0 file:bg-[#d6a72f] file:px-3 file:py-2 file:text-sm file:font-semibold file:text-[#16120c] disabled:opacity-50" />
          </label>
          <div className="mt-5 space-y-3">
            {(Object.keys(consentLabels) as ConsentKey[]).map((key) => (
              <label key={key} className="flex gap-3 rounded-md border border-[#d6a72f]/20 bg-white/50 p-3 text-sm text-[#5f3f16]">
                <input type="checkbox" checked={consent[key]} onChange={(event) => setConsent((current) => ({ ...current, [key]: event.target.checked }))} className="mt-1 accent-[#d6a72f]" />
                <span>{consentLabels[key]}</span>
              </label>
            ))}
          </div>
          {error && <p className="mt-4 rounded-md border border-red-300/60 bg-red-50 p-3 text-sm text-red-700">{error}</p>}
          <button disabled={!verified || !file || !allConsent || busy} onClick={() => void uploadAndAnalyze()} className="gold-button mt-5 w-full">
            {busy ? "Uploading privately..." : "Upload for private analysis"}
          </button>
        </Panel>
        <Panel>
          <p className="kicker">Private status</p>
          <h2 className="mt-2 text-xl font-semibold">Vault processing</h2>
          {!state ? (
            <div className="mt-5 grid gap-3">
              {["Await verified upload", "Record consent", "Moderate", "Analyze with Grok", "Show owner-only result"].map((item, index) => (
                <div key={item} className="flex items-center gap-3 rounded-md border border-[#d6a72f]/20 bg-white/50 p-3 text-sm text-[#5f3f16]">
                  <span className="flex h-7 w-7 items-center justify-center rounded-md bg-[#fff8e6] font-semibold">{index + 1}</span>
                  {item}
                </div>
              ))}
            </div>
            ...
            <div className="mt-3">
              <p className="text-sm text-[#5f3f16]">Analysis completed.</p>
              <Link href={`/results/${state.result.id}`} className="gold-button mt-4">
                View private result
              </Link>
            </div>
          ) : state.upload?.status === "quarantined" || state.upload?.moderationStatus === "rejected" ? (
            <p className="mt-3 text-sm text-[#5f3f16]">This upload could not be processed. Please review the safety guidance or contact support.</p>
          ) : (
            <p className="mt-3 rounded-md border border-[#d6a72f]/20 bg-white/50 p-3 text-sm text-[#5f3f16]">Upload status: {state.upload?.status}. Moderation: {state.upload?.moderationStatus}.</p>
          )}
        </Panel>
      </main>
    </PhaseShell>
